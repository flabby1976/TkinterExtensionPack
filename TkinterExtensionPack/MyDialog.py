...
if __name__ == "__main__":
    ...
    class PopupWindow(MyDialog):
        ...
        def body(self, master):
            ...
            Label(master, text="Segment").grid(row=0, column=0)
            self.var = StringVar(master)
            self.var.set(seg_name_list[0])  # initial value

            option = OptionMenu(master, self.var, *seg_name_list)

            Label(master, text="Name").grid(row=1, column=0)
            self.E1 = Entry(master)

            Label(master, text="Distance").grid(row=2, column=0)
    ...
    wz = PopupWindow(root, title="Insert RPL Event")

    try:
        (zmy_seg, zmy_entry_name, zmy_distance) = wz.result
    except TypeError:
        # This probably means that Cancel was pressed (w.result = None)
        pass

    log.debug("%s %s %s" % (zmy_seg, zmy_entry_name, zmy_distance))
